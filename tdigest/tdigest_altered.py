"""
Altered tdigest class.

Based on this implementation: https://github.com/CamDavidsonPilon/tdigest

Merging: __add__ method altered to allow for digest combination
        that inserts centroids in sorted order by mean and sorted order
        by centroid size (in addition to default random insertion).

Future work should include better compression control - currently
compression is done in ad-hoc manner that "plateaus" after ~50
compression runs - works in in practice, but something more explicit
about size requirements may be desirable.
"""
